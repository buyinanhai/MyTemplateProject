...
public extension UIView{
    ...
    func dy_addCornersForMask(radius:CGFloat) {
        
        let size = CGSize.init(width: radius, height: radius);
        let shapeLayer = CAShapeLayer.init();
        shapeLayer.path = UIBezierPath.init(roundedRect: self.bounds, byRoundingCorners: .allCorners, cornerRadii: size).cgPath;
...
public extension Date {
    ...
    static func getFormdateMDHM(timeStamp: Double) -> String {
        let timeSta:TimeInterval = TimeInterval(timeStamp / 1000)
    ...
    static func getFormdateHM(timeStamp: Double) -> String {
        let timeSta:TimeInterval = TimeInterval(timeStamp / 1000)
